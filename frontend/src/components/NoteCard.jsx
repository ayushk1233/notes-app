...
const noteColors = [
  { 
    name: 'Default', 
    value: 'bg-white/80 dark:!bg-[#9da5bd]'
  },
  { 
    name: 'Lighter', 
    value: 'bg-sky-50/80 dark:!bg-[#9da5bd]/95'
  },
  { 
    name: 'Light', 
    value: 'bg-blue-50/80 dark:!bg-[#9da5bd]/90'
  },
  { 
    name: 'Soft', 
    value: 'bg-indigo-50/80 dark:!bg-[#9da5bd]/85'
  },
  { 
    name: 'Medium', 
    value: 'bg-white/60 dark:!bg-[#9da5bd]/80'
  },
  { 
    name: 'Glass', 
    value: 'bg-white/60 dark:!bg-[#9da5bd]/75'
  },
  { 
    name: 'Frost', 
    value: 'bg-blue-50/60 dark:!bg-[#9da5bd]/70'
  },
  { 
    name: 'Clear', 
    value: 'bg-white/50 dark:!bg-[#9da5bd]/65'
  },
];
...
const NoteCard = ({ title, content, tags = [], backgroundColor = 'bg-yellow-100', onView, onEdit, onDelete, onShare }) => {
  ...
  const getInitialHeight = () => {
    ...
    if (!hasMoreContent) return 'auto';
    const lineCount = (content.match(/\n/g) || []).length + 1;
    const baseHeight = Math.min(lineCount * 24, 300); // 24px per line, max 300px
    return isExpanded ? 'auto' : `${baseHeight}px`;
  };
  ...
  return (
    <motion.div
      ref={cardRef}
      className={`
        group relative rounded-xl transform-gpu
        p-4 shadow-lg backdrop-blur-sm
        ${backgroundColor || 'bg-white/90'}
        dark:!bg-[#9da5bd]
        hover:shadow-xl transition-all duration-300
        border-0 ring-1 ring-black/5 dark:ring-[#9da5bd]/30
        hover:ring-black/10 dark:hover:ring-[#9da5bd]/50
        dark:shadow-lg dark:shadow-gray-900/30
      `}
      whileHover={{ 
        scale: 1.02,
    ...
    >
      <div 
        className={`h-full rounded-xl p-6 transition-all duration-200 backdrop-blur-sm
          bg-transparent
          hover:shadow-md dark:hover:shadow-lg dark:shadow-black/20`}
      >
        <h3 
          className={`font-semibold text-xl mb-3 leading-tight line-clamp-2
            text-light-text dark:text-dark-text tracking-tight`}
        >
          {title}
        </h3>
        ...
        >
          <div 
            className={`whitespace-pre-line text-base leading-relaxed font-medium
              text-light-text/90 dark:text-dark-text/90`}
          >
            {hasMoreContent && !isExpanded ? (
              <>
                {content.slice(0, contentPreviewLength)}
